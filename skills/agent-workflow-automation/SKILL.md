...
> 
> **前置知识**：使用本技能前，请确保已熟悉基础 Agent 协调规范 → [agent-coordination](../agent-coordination/SKILL.md)
...
| 场景 | 说明 | 示例 |
|------|------|------|
| **并行审查** | Guardian + Inspector 同时审查 | 代码提交后自动触发 |
| **前后端并行** | 后端 API + 前端 UI 同时开发 | 使用 Mock 数据解耦 |
| **多模块并行** | 大型项目拆分为独立模块 | Header/Sidebar/Content 并行 |
| **数据处理并行** | 多个文件/数据集同时处理 | 批量 CSV 处理 |
| **多环境部署** | 开发/测试/生产环境同时部署 | CI/CD 流水线 |
