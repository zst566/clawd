...
| 事项 | 说明 | 状态 |
|------|------|------|
| **润德教育3.0数据清理** | 发现9万+条异常记录，涉及1万+订单ID，需重新计算摊销数据，完成后统计年度汇总表给财务核对 | ⏳ 待进行 |

#### 📋 今日待办
| 事项 | 说明 | 状态 |
|------|------|------|
| 山西培训平台小程序设计修改 | 客户积分 + 积分兑换功能 | ⏳ 待进行 |
| **润德教育12月财务数据核对** | 🔴 **发现严重问题**：财务订单有数据但清洗库缺少数据，基础数据同步错误导致，明天继续检查确定原因并补齐数据 | 🔴 明天完成 |
| 茂名文旅小程序注册备案 | 本周启动 | ⏳ 待进行 |
| PC端页面与后端API联调 | Banner管理页面对接 | ⏳ 待进行 |
| **图片上传功能对接** | 待实现 | ⏳ 待进行 |
...
| 日期 | 事项 | 说明 |
|------|------|------|
| **1月31日前** | 润德教育12月财务数据核对 | 🔴 **发现严重问题**：财务订单有数据但清洗库缺少数据，基础数据同步错误导致，明天继续检查确定原因并补齐数据 |
...
- [ ] **山西培训平台** - 客户积分功能 + 积分兑换入口设计 | ⏳ 待设计 |
  ...
  - 需修改小程序端设计，添加积分和兑换模块 |
- [ ] **润德教育3.0数据清理** - 🔴 **紧急** |
  - 问题：9万+条异常记录，涉及1万+订单ID |
  - 任务：重新计算这1万多个订单的摊销数据 |
  - 完成后：统计每个年度的"本年新增实收"、"本年确认收入"、"本年期末余额"，汇总成表格给财务核对 |
- [ ] **润德教育12月财务核对** - 🔴 **发现严重问题**：财务订单有数据但清洗库缺少数据，基础数据同步错误导致，需明天继续检查确定原因并补齐数据 | 🔴 **1月31日前** |
...
| 2026-01-30 | 新增润德教育3.0数据清理任务：9万+异常记录，涉及1万+订单ID，需重新计算并统计年度汇总表给财务核对 | 小d |
| 2026-01-30 | 润德教育12月财务核对更新：发现财务订单有数据但清洗库缺少数据，基础数据同步错误导致，需明天继续检查确定原因并补齐数据 | 小d |
